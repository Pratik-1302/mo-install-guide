...
A single-host miniOrange + MSSQL deployment is not possible on Debian 11. Two viable workarounds:

1. **Use a different OS for the IDP.** Ubuntu 22.04 is the closest behavioural match to Debian 11; if MSSQL is a hard requirement, switching the IDP host to Ubuntu 22.04 keeps your apt-based ops workflow intact and unlocks the supported MSSQL path.
2. **Run MSSQL on a separate host.** Install MSSQL on RHEL, SLES, or Ubuntu, then on this Debian 11 IDP host place only the JDBC driver and point the wizard at the remote DB.

### JDBC-driver-only path on Debian 11
